摘自：`https://www.javazhiyin.com/34620.html`
...
详细的配置文件文件内容可以到Tomcat官网查看。上边的配置文件，还可以通过下边的一张结构图更清楚的理解：
...
我们访问应用Context的时候，如果是ROOT下的则直接使用域名就可以访问，例如：`www.ledouit.com`,如果是Host（webapps）下的其他应用，则可以使用`www.ledouit.com/docs`进行访问，当然默认指定的根应用（ROOT）是可以进行设定的，只不过Host站点下默认的主营用是ROOT目录下的。
