...
## Thread Two Phase Termination 模式

比如我们在结束一个线程的时候，可能这个线程还需要做一些收尾工作，而不能立即停止。
...
下面通过一个网络请求模拟一下此种设计方式

```java
//一个server监听某个端口，来接受网络请求，可以由多个客户端来连接
package concurrent.phase2;
...
这里在测试的时候`windows`上面可以使用`telnet`来发送请求，在`mac`上面可以使用

```shell
nc localhost 12722
```

然后当我们将服务端关闭的时候，服务端会将相关内容进行清理。



## Work Thread 模式

这种模式就类似于一个工厂，有一条生产线，这条生产线有一定数量的工人，然后会有人将相关的货物放在这条生产线上或者叫传送带上，然后工人会从传送带上取货物去处理。
...
这种模式就类似于`GC`，在程序运行的过程中，如果我们调用`GC`，即给`GC`线程发送命令，`GC`则会帮我们执行，此命令执行完后并不会停止，还会继续接受命令来执行，同时每次发送的命令可能还是不同的，相当于每次需要执行不同的方法，如果我们单独使用`Future`是无法完成的。

参考：`https://www.ibm.com/developerworks/cn/java/j-lo-activeobject/index.html`
...
一般来说，并发对象的设计实现需要考虑下面的几个重要因素：

- 并发对象的任何一次的方法执行，不允许无限地或者长时间阻止其它方法的调用执行，从而影响应用的 `QoS(Quality of Service)`。
...
实现的思路是，通过代理将方法的调用转变为向阻塞队列中添加一个请求，由一个线程取出请求后执行实际的方法，然后将结果设置到`Future`中，当然也可以直接返回。

**适用的情况**

`actor`是一个很抽象多线程模式, 每一个`actor`是线程独立并且有属于自己的状态, 多个`actor`互相发送消息以完成最终的任务. 你可以将`actor`模式理解为一个团队, 这个团队里面的个人就是一个运行在独立线程上的个体, 由他们互相交流并且单独处理自己的工作, 最后完成任务.
 关于`actor`模式, 这里有一篇个人觉得很好的文章(英文)--[The actor model in 10 minutes](https://links.jianshu.com/go?to=https%3A%2F%2Fwww.brianstorti.com%2Fthe-actor-model%2F)
...
这是一个接口，这里我们假设会发两种命令，具体命令执行由下面类完成
...
但是我们这里不会直接调用上面的类来执行，因为这里是由其他线程来执行，于是需要一个代理，代理中由其他线程来调用，然后将结果和调用线程绑定
...
可以很明显看到，方法的具体执行由`SchedulerThread`来完成
...
而要知道方法是异步执行的，这就和生产者消费者模式有点类似了，需要一个队列来存放相关的线程调用
...
从上面可以看到这里模拟了两种请求，一种是普通调用，一种是使用了`Future`。
