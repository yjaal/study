...
常见的缓存问题有以下几个：
- [缓存与数据库双写不一致](./缓存与数据库双写不一致.md)
- [缓存雪崩、缓存穿透](./缓存雪崩、缓存穿透.md)
- [缓存并发竞争](./缓存并发竞争.md)
...
redis 相比 memcached 来说，拥有[redis数据结构](./redis数据结构.md)，能支持更丰富的数据操作。如果需要缓存能够支持更复杂的结构和操作， redis 会是不错的选择。
...
# redis 都有哪些数据类型？分别在哪些场景下使用比较合适？以及使用C语言哪种数据结构实现的？
